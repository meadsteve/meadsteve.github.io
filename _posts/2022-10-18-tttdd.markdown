...
I created a new test file `test/graphql/blogposts-graphql.e2e-spec.ts` and wrote a test that the query earlier 
should result in `[]` blogposts:
...
But first I needed to go back to my types and make a mutation available. 
...
storage class has some test of its own.)

```
0 failing tests, 6 passing
```

And this is where I'm stopping the experiment for now. But with this way of working I should be able to continue looping
between "writing some typed code", "writing a test" and "making the test pass" to add any other features I want. 